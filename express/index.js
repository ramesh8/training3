...
app.post("/login", async (req, res) => {
  ...
  if (passwordMatch) {
    ...
    req.session.message = "Welcome, " + foundUser.name;
    res.redirect("/");
    return;
  } else {
...
app.get("/collab", async (req, res) => {
  ...
  // console.log(req.session);
  let users = await (
    await db.collection("users").find({}, "-password")
  ).toArray();
  let collabs = await (await db.collection("collab").find()).toArray();
  let projects = await (await db.collection("projects").find()).toArray();
